...
import { sendManualPaymentEmail, sendTicketEmail } from "@/lib/mail";
import {
...
export async function POST(request) {
    ...
    const totals = calculateBookingTotals(selectedTicketType.price, quantity);
    ...
    if (!isFreeBooking && (!billingName || !billingStreet || !billingPostalCode || !billingCity)) {
    ...
    if (booking) {
        ...
        try {
            booking = await prisma.$transaction(async (tx) => {
                ...
                if (delta !== 0 || previousTicketTypeId !== nextTicketTypeId || paymentMethodChanged) {
    ...
    if (isFreeBooking) {
        ...
        sendTicketEmail(paidBooking).catch((error) => {
            console.error("Free ticket mail error:", error);
        });
